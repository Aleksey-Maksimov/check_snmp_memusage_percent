#!/bin/sh
#
# Icinga Plugin Script (Check Command). It calculate the percentage of memory usage from received SNMP data
...
#
PLUGIN_NAME="Icinga Plugin Check Command to calculate the percentage of memory used (from SNMP data)"
...
vPERCENT=$( awk "BEGIN { pc=100*${vUSED}/${vMTOTAL}; i=int(pc); print (pc-i<0.5)?i:i+1 }" )
#
# Icinga Check Plugin output
#
if [ "$vPERCENT" -ge "$LIMITCRITICAL" ]; then
    echo "Memory Usage percent CRITICAL - $vPERCENT % | MemoryUsagePercent=$vPERCENT"
    exit $codeCRITICAL
elif [ "$vPERCENT" -ge "$LIMITWARNING" ]; then
    echo "Memory Usage percent WARNING - $vPERCENT % | MemoryUsagePercent=$vPERCENT"
    exit $codeWARNING
elif [ "$vPERCENT" -lt "$LIMITWARNING" ]; then
    echo "Memory Usage percent OK - $vPERCENT % | MemoryUsagePercent=$vPERCENT"
    exit $codeOK
fi
exit $codeUNKNOWN
